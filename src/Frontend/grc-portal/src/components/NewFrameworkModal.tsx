...
export function NewFrameworkModal({ onClose, onSave }: NewFrameworkModalProps) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.7)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <div style={{ background:'#ffffff', borderRadius:16, width:'100%', maxWidth:560, border:'1px solid #e6e7de', boxShadow:'0 24px 80px rgba(20,23,13,0.14)', display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'24px 28px 16px', borderBottom:'1px solid #e6e7de', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
          <div>
            <h2 style={{ fontSize:20, fontWeight:800, color:'#14170d', marginBottom:4 }}>New Compliance Framework</h2>
            <p style={{ fontSize:12, color:'#6b7060' }}>ISO 27001:2022 — Clause 6.1.3 Compliance Tracking</p>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
            <div>
              ...
              <select style={inp} value={form.type} onChange={e => set('type', Number(e.target.value))}>
                ...
                {FRAMEWORK_TYPES.map(t => <option key={t.value} value={t.value} style={{ background:'#f4f5ee' }}>{t.label}</option>)}
              </select>
            </div>
            <div>
