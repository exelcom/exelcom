...
export function NewAuditModal({ onClose, onSave }: NewAuditModalProps) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#ffffff', borderRadius: 16, width: '100%', maxWidth: 620, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', border: '1px solid #e6e7de', boxShadow: '0 24px 80px rgba(20,23,13,0.14)' }}>
        ...
        <div style={{ padding: '24px 28px 16px', borderBottom: '1px solid #e6e7de', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: '#14170d', marginBottom: 4 }}>New Audit</h2>
            <p style={{ fontSize: 12, color: '#6b7060' }}>ISO 27001:2022 — Clause 9.2 Internal Audit</p>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {AUDIT_TYPES.map(t => (
                <button key={t.value} type="button" onClick={() => set('type', t.value)} style={{ padding: '14px 16px', borderRadius: 10, cursor: 'pointer', textAlign: 'left', border: `1px solid ${form.type === t.value ? '#10b981' : '#e6e7de'}`, background: form.type === t.value ? '#10b98120' : '#f4f5ee', transition: 'all 0.15s' }}>
                  <div style={{ fontSize: 18, marginBottom: 4 }}>{t.icon}</div>
                  <div style={{ fontSize: 13, fontWeight: form.type === t.value ? 700 : 500, color: form.type === t.value ? '#10b981' : '#14170d' }}>{t.label}</div>
                  <div style={{ fontSize: 11, color: '#6b7060', marginTop: 2 }}>{t.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {form.startDate && form.endDate && new Date(form.endDate) > new Date(form.startDate) && (
            <div style={{ padding: 14, background: '#f4f5ee', borderRadius: 10, border: '1px solid #e6e7de', display: 'flex', gap: 20 }}>
              <div>
                <div style={{ fontSize: 11, color: '#6b7060' }}>Duration</div>
              ...
              <div>
                <div style={{ fontSize: 11, color: '#6b7060' }}>Type</div>
        ...
        <div style={{ padding: '16px 28px', borderTop: '1px solid #e6e7de', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button onClick={onClose} style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid #e6e7de', background: 'none', color: '#6b7060', cursor: 'pointer', fontSize: 14 }}>Cancel</button>
