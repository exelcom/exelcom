...
public class Risk : BaseAuditableEntity
{
    ...
    public int? ResidualScore => ResidualLikelihood.HasValue && ResidualImpact.HasValue ? (int)ResidualLikelihood.Value * (int)ResidualImpact.Value : null;
    public string RiskRating => InherentScore switch { >= 20 => "Critical", >= 12 => "High", >= 6 => "Medium", _ => "Low" };
    ...
    private Risk() { }
    public static Risk Create(string title, string description, RiskCategory category, RiskLikelihood likelihood, RiskImpact impact, string createdBy, string? owner = null, string? department = null, DateTime? reviewDueDate = null, string? regulatoryReference = null)
    {
        var risk = new Risk { Title = title, Description = description, Category = category, Status = RiskStatus.Identified, InherentLikelihood = likelihood, InherentImpact = impact, Owner = owner, Department = department, ReviewDueDate = reviewDueDate, RegulatoryReference = regulatoryReference, CreatedBy = createdBy };
