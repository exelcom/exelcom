...
public class RisksController(IMediator mediator) : ControllerBase
{
    ...
    [HttpPost] public async Task<ActionResult<RiskDto>> CreateRisk([FromBody] CreateRiskRequest req, CancellationToken ct) { var r = await mediator.Send(new CreateRiskCommand(req.Title, req.Description ?? "", Enum.Parse<RiskCategory>(req.Category), Enum.Parse<RiskLikelihood>(req.Likelihood), Enum.Parse<RiskImpact>(req.Impact), req.Owner, req.Department, req.ReviewDueDate, req.RegulatoryReference), ct); return CreatedAtAction(nameof(GetRisk), new { id = r.Id }, r); }
...
public record CreateRiskRequest(string Title, string? Description, string Category, string Likelihood, string Impact, string? Owner, string? Department, DateTime? ReviewDueDate, string? RegulatoryReference);
