...
public class ComplianceFramework : BaseAuditableEntity
{
    ...
    public int ImplementedControls => _controls.Count(c =>
        c.Status == ControlStatus.Implemented || c.Status == ControlStatus.Verified);
    public double CompliancePercentage => TotalControls == 0 ? 0 :
        Math.Round((double)ImplementedControls / TotalControls * 100, 1);

    private ComplianceFramework() { }

    public static ComplianceFramework Create(string name, string description,
        FrameworkType type, string version, string createdBy) => new()
    {
        Name = name,
    ...
    public void AddControl(string controlId, string title, string description,
        string? guidance, string createdBy)
    {
        var control = ComplianceControl.Create(Id, controlId, title, description, guidance, createdBy);
...
public class ComplianceControl : BaseAuditableEntity
{
    ...
    public string ControlId { get; private set; } = string.Empty;  // e.g. "A.8.1.1"
    ...
    public static ComplianceControl Create(Guid frameworkId, string controlId, string title,
        string description, string? guidance, string createdBy) => new()
    {
    ...
    public void UpdateImplementation(ControlStatus status, CompliancePosture posture,
        string? notes, string? owner, DateTime? nextReviewDate, string updatedBy)
    {
        ...
        if (previousStatus != status)
            _domainEvents.Add(new ControlStatusChangedEvent(Id, FrameworkId, previousStatus, status, updatedBy));
    }

    public void AddEvidence(string fileName, string blobUrl, EvidenceType type,
        string description, string uploadedBy)
    {
        _evidence.Add(ControlEvidence.Create(Id, fileName, blobUrl, type, description, uploadedBy));
        UpdatedAt = DateTime.UtcNow;
...
public class ControlEvidence : BaseEntity
{
    ...
    public static ControlEvidence Create(Guid controlId, string fileName, string blobUrl,
        EvidenceType type, string description, string uploadedBy) => new()
    {
        ControlId = controlId,
        FileName = fileName,
        BlobUrl = blobUrl,
