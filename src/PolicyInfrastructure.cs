...
public class PolicyDbContext(DbContextOptions<PolicyDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<PolicyVersion>(e =>
        {
            ...
            e.Property(v => v.ChangeNotes).HasMaxLength(1000).IsRequired();
            e.Property(v => v.CreatedBy).HasMaxLength(200).IsRequired();
        });

        modelBuilder.Entity<PolicyAttestation>(e =>
        {
            e.HasKey(a => a.Id);
            e.Property(a => a.UserId).HasMaxLength(200).IsRequired();
            e.Property(a => a.UserEmail).HasMaxLength(300).IsRequired();
            e.Property(a => a.CreatedBy).HasMaxLength(200).IsRequired();
            e.HasIndex(a => new { a.PolicyId, a.UserId });
        });
    }
}
...
public class PolicyRepository(PolicyDbContext context) : IPolicyRepository
{
    public async Task<Policy?> GetByIdAsync(Guid id, CancellationToken ct = default)
        => await context.Policies.FirstOrDefaultAsync(p => p.Id == id, ct);

    public async Task<Policy?> GetByIdWithDetailsAsync(Guid id, CancellationToken ct = default)
        => await context.Policies
            .Include(p => p.Versions)
            .Include(p => p.Attestations)
            .FirstOrDefaultAsync(p => p.Id == id, ct);

    public async Task<PagedResult<Policy>> GetPagedAsync(int page, int pageSize,
        PolicyStatus? status = null, PolicyCategory? category = null, CancellationToken ct = default)
    {
        var query = context.Policies.Include(p => p.Versions).AsQueryable();
        if (status.HasValue) query = query.Where(p => p.Status == status.Value);
        if (category.HasValue) query = query.Where(p => p.Category == category.Value);
        var total = await query.CountAsync(ct);
        var items = await query.OrderByDescending(p => p.CreatedAt)
            .Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(ct);
        return PagedResult<Policy>.Create(items, total, page, pageSize);
    }
